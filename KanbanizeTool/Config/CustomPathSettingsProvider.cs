...
namespace KanbanizeTool.Config
{
    ...
    /// TODO: Update summary.
    /// </summary>
    public class CustomPathSettingsProvider : SettingsProvider ///ApplicationSettingsBase
    {
        ...
        /// </summary>
        public CustomPathSettingsProvider()
        {
        ...
        public override SettingsPropertyValueCollection GetPropertyValues(SettingsContext context, SettingsPropertyCollection collection)
        {
            ...
            foreach (SettingsProperty setting in collection)
            {
                ...
                if (SettingsDictionary.ContainsKey(setting.Name))
                {
                    ...
                    if (ShouldEncrypt(setting))
                    {
                        value.PropertyValue = Crypto.ToInsecureString(Crypto.DecryptString(SettingsDictionary[setting.Name].value));
                    }
                    else
                    {
                        try
                        ...
                        catch
                        {
                            if (t == typeof(Color))
                            {
                                DeserializeColorAndSetValue(value, setting);
                            }
                        }
                    }
                }
                else //use defaults in the case where there are no settings yet
                {
                    ...
                    catch
                    {
                        if (t == typeof(Color))
                        {
                            value.PropertyValue = Color.FromName(setting.DefaultValue as string);
                        }
                    }
                }
        ...
        private void DeserializeColorAndSetValue(SettingsPropertyValue value, SettingsProperty setting)
        {
            var color = Color.FromName(SettingsDictionary[setting.Name].value);
            if (color.IsKnownColor || color.IsSystemColor)
                value.PropertyValue = color;
            else
                value.PropertyValue =
                    ColorTranslator.FromHtml(SettingsDictionary[setting.Name].value);
        }
        ...
        static bool ShouldEncrypt(SettingsProperty settingsProperty)
        {
            var t = Type.GetType(settingsProperty.PropertyType.FullName);
            //if (t == typeof(System.Security.SecureString))
            if (settingsProperty.Name.Contains("Password"))
            {
                return true;
            }
            return false;
        ...
        private void CreateEmptyConfig()
        {
            ...
            using (File.Create(UserConfigPath)) { }
        ...
        /// </summary>
        internal void SaveValuesToFile()
        {
        ...
        private string UserConfigPath
        {
            get
            {
                var filePath = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) + Path.DirectorySeparatorChar +
                    Properties.Settings.Default.SettingsKey + Path.DirectorySeparatorChar +
                    "user.config";
                return filePath;
            }
